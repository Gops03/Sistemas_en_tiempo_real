...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		// Registro de la ruta del URI
        httpd_uri_t set_rgb_uri = {
        .uri       = "/set_rgb",
        .method    = HTTP_POST,
        .handler   = set_rgb_handler,  // Asociamos la función de manejo set_rgb_handler con esta ruta
        .user_ctx  = NULL
        };
		// Luego, registra la ruta en el servidor
        httpd_register_uri_handler(http_server_handle, &set_rgb_uri);
