...
#define LED_DUTY_RES           LEDC_TIMER_14_BIT
#define LED_FREQUENCY          (3000)
...
char on_resp[] = "<!DOCTYPE html><html><head><style type=\"text/css\">html {  font-family: Arial;  display: inline-block;  margin: 0px auto;  text-align: center;}h1{  color: #070812;  padding: 2vh;}.button {  display: inline-block;  background-color: #b30000; //red color  border: none;  border-radius: 4px;  color: white;  padding: 16px 40px;  text-decoration: none;  font-size: 30px;  margin: 2px;  cursor: pointer;}.button2 {  background-color: #364cf4; //blue color}.content {   padding: 50px;}.card-grid {  max-width: 800px;  margin: 0 auto;  display: grid;  grid-gap: 2rem;  grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));}.card {  background-color: white;  box-shadow: 2px 2px 12px 1px rgba(140,140,140,.5);}.card-title {  font-size: 1.2rem;  font-weight: bold;  color: #034078}</style>  <title>ESP32 WEB SERVER</title>  <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">  <link rel=\"icon\" href=\"data:,\">  <link rel=\"stylesheet\" href=\"https://use.fontawesome.com/releases/v5.7.2/css/all.css\"    integrity=\"sha384-fnmOCqbTlWIlj8LyTjo7mOUStjsKC4pOpQbqyi7RrhN7udi9RwhKkMHpvLbHG9Sr\" crossorigin=\"anonymous\">  <link rel=\"stylesheet\" type=\"text/css\"></head><body>  <h2>ESP32 WEB SERVER</h2>  <div class=\"content\">    <div class=\"card-grid\">      <div class=\"card\">        <p><i class=\"fas fa-lightbulb fa-2x\" style=\"color:#c81919;\"></i>     <strong>GPIO2</strong></p>        <p>GPIO state: <strong> ON</strong></p>        <p>          <a href=\"/led2on\"><button class=\"button\">ON</button></a>          <a href=\"/led2off\"><button class=\"button button2\">OFF</button></a>        </p>        <!-- Potenciómetro -->        <p>Intensidad: <input type=\"range\" min=\"0\" max=\"100\" value=\"50\" class=\"slider\" id=\"intensidad\"></p>      </div>    </div>  </div></body></html>";

//char on_resp[] = "<!DOCTYPE html><html><head><style type=\"text/css\">html {  font-family: Arial;  display: inline-block;  margin: 0px auto;  text-align: center;}h1{  color: #070812;  padding: 2vh;}.button {  display: inline-block;  background-color: #b30000; //red color  border: none;  border-radius: 4px;  color: white;  padding: 16px 40px;  text-decoration: none;  font-size: 30px;  margin: 2px;  cursor: pointer;}.button2 {  background-color: #364cf4; //blue color}.content {   padding: 50px;}.card-grid {  max-width: 800px;  margin: 0 auto;  display: grid;  grid-gap: 2rem;  grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));}.card {  background-color: white;  box-shadow: 2px 2px 12px 1px rgba(140,140,140,.5);}.card-title {  font-size: 1.2rem;  font-weight: bold;  color: #034078}</style>  <title>ESP32 WEB SERVER</title>  <meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">  <link rel=\"icon\" href=\"data:,\">  <link rel=\"stylesheet\" href=\"https://use.fontawesome.com/releases/v5.7.2/css/all.css\"    integrity=\"sha384-fnmOCqbTlWIlj8LyTjo7mOUStjsKC4pOpQbqyi7RrhN7udi9RwhKkMHpvLbHG9Sr\" crossorigin=\"anonymous\">  <link rel=\"stylesheet\" type=\"text/css\"></head><body>  <h2>ESP32 WEB SERVER</h2>  <div class=\"content\">    <div class=\"card-grid\">      <div class=\"card\">        <p><i class=\"fas fa-lightbulb fa-2x\" style=\"color:#c81919;\"></i>     <strong>GPIO2</strong></p>        <p>GPIO state: <strong> ON</strong></p>        <p>          <a href=\"/led2on\"><button class=\"button\">ON</button></a>          <a href=\"/led2off\"><button class=\"button2\">OFF</button></a>        </p>      </div>      <div class=\"card\">        <p>Intensidad: <input type=\"range\" min=\"0\" max=\"100\" value=\"50\" class=\"slider\" id=\"intensidad\"></p>      </div>    </div>  </div></body></html>";
...
//FUNCIONES DE inICIALIZACION DE LOS LEDS...
void leds_init(void)
{
    ...
    ledc_channel_config_t ledc_channel = {
        ...
        .gpio_num       = LED_PIN,
        .duty           = (1 << LED_DUTY_RES) - 1,  // Establece el deber al 100%
        .hpoint         = 0
    };
    ESP_ERROR_CHECK(ledc_channel_config(&ledc_channel));
}

esp_err_t set_intensity_handler(httpd_req_t *req)
{
    char *buf;
    size_t buf_len;
    int intensity = 50; // Valor por defecto

    // Obtener el valor de intensidad del parámetro "intensidad" en la URL
    if (httpd_req_get_url_query_str(req, &buf, &buf_len) == ESP_OK)
    {
        int max_len = 4; // Número máximo de dígitos esperados para intensidad
        char intensity_str[max_len + 1];
        if (httpd_query_key_value(buf, "intensidad", intensity_str, max_len) == ESP_OK)
        {
            intensity = atoi(intensity_str); // Convertir a entero
        }
    }

    // Asegúrate de que la intensidad esté en el rango correcto (0-100)
    if (intensity < 0)
    {
        intensity = 0;
    }
    ...
    uint32_t duty = (intensity * ((1 << LED_DUTY_RES) - 1)) / 100;

    // Configura el ciclo de trabajo para el canal LEDC y actualiza
    ledc_set_duty(LED_MODE, LED_CHANNEL, duty);
    ledc_update_duty(LED_MODE, LED_CHANNEL);

    // Enviar respuesta HTML al navegador
    char response[100];
    snprintf(response, sizeof(response), "Intensidad configurada a %d%%", intensity);
...
httpd_uri_t uri_set_intensity = {
    .uri = "/setIntensity",
    .method = HTTP_GET,
    .handler = set_intensity_handler,
    .user_ctx = NULL};
...
httpd_handle_t setup_server(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(server, &uri_set_intensity);
...
{
    leds_init(); 
    ledc_set_duty(LED_MODE, LED_CHANNEL, 8);
    // Initialize NVS
